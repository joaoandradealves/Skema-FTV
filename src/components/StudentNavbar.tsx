  ...
  return (
    <nav className="fixed bottom-0 left-0 w-full flex justify-around items-center px-4 pb-6 pt-3 bg-[#FFF8F3]/70 backdrop-blur-md z-50 rounded-t-3xl shadow-[0px_-10px_30px_rgba(164,60,18,0.05)]">
      <Link to="/student" className={activePage === 'home' ? activeClass : inactiveClass}>
        ...
        <span className="font-body text-[10px] font-semibold uppercase tracking-wider mt-0.5">Início</span>
      </Link>
      <Link to="/student/history" className={activePage === 'agenda' ? activeClass : inactiveClass}>
        <span className="material-symbols-outlined" style={activePage === 'agenda' ? { fontVariationSettings: "'FILL' 1" } : undefined}>history_edu</span>
        <span className="font-body text-[10px] font-semibold uppercase tracking-wider mt-0.5">Histórico</span>
      </Link>
      <Link to="/profile" className={activePage === 'perfil' ? activeClass : inactiveClass}>
        ...
        <span className="font-body text-[10px] font-semibold uppercase tracking-wider mt-0.5">Perfil</span>
      </Link>
    </nav>
  );
}
